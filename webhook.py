...
class WebHook:
    ...
    def format_with_template(self, company_obj):
        with open('../msg_template', 'r', encoding="utf8") as file:
            data = file.read().replace('%job_title%', company_obj.job_title).replace('%company_name%', company_obj.company_name).replace('%location%', company_obj.location)\
                .replace('%company_url%', company_obj.company_url)
            file.close()
            return data
    # def get_url(self):
    ...
    #     sign = urllib.parse.quote_plus(b64base64.encode(hmac_code))
    #
    #     # 完整的url
